...
resource "aws_ses_domain_mail_from" "ses_domain_mail_from" {
  ...
  depends_on = [aws_route53_record.spf_mail_from,aws_route53_record.mx_send_mail_from,aws_ses_domain_identity.ses_domain_identity]
}

resource "aws_route53_record" "spf_mail_from" {
  zone_id = var.zone_id
  name    = local.stripped_mail_from_domain
  type    = var.txt_type
  ttl     = var.ttl
